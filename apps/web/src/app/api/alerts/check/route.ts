...
// Classify current regime from the latest block's blob_count vs max slots
...
export async function POST() {
  try {
    ...
    // Get the current regime from the latest block stat
    ...
    // Fetch enabled alerts where severity threshold is met
    ...
    await Promise.all(
      alerts.map(async (alert) => {
        ...
        if (
          currentSeverity < minSeverity ||
          alert.last_fired_regime === currentRegime
        ) {
          skipped++;
          return;
        }
        ...
        try {
          ...
          if (res.ok) {
            await sql`
              ...
              SET last_fired_regime = ${currentRegime},
                  last_fired_at = NOW()
              WHERE id = ${alert.id}
            `;
            fired++;
    ...
    const result: CheckResult = { regime: currentRegime, fired, skipped, errors };
