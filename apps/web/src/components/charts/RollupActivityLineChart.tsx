...
export function RollupActivityLineChart({ data }: Props) {
  ...
  if (!data.length)
    return <p className="py-8 text-center text-[0.6875rem] text-text-secondary opacity-50 italic">No historical activity data</p>;
          ...
          <div style="display:flex;flex-direction:column;gap:8px;min-width:180px;">
            ...
            ${rows.map(p => `
              <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
                ...
                <span style="font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:bold;color:${p.color}">${p.value} <span style="font-size:9px;opacity:0.5;">blobs</span></span>
              </div>
            `).join('')}
